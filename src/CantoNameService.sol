  ...
  LinearVRGDA
{
  ...
  modifier onlyNameOwner(string memory _name) {
    ...
    require(nameOwner[id] != address(0), "NOT_MINTED");
  ...
  function vrgdaBatchInitialize() public onlyContractOwner {
    ...
    require(
      vrgdaBatch.oneTargetPrice > 0 &&
      vrgdaBatch.onePriceDecayPercent > 0 &&
      vrgdaBatch.onePerTimeUnit > 0,
      "VRGDA_ONE_MISSING_DATA"
    );
    require(
      vrgdaBatch.twoTargetPrice > 0 &&
      vrgdaBatch.twoPriceDecayPercent > 0 &&
      vrgdaBatch.twoPerTimeUnit > 0,
      "VRGDA_TWO_MISSING_DATA"
    );
    require(
      vrgdaBatch.threeTargetPrice > 0 &&
      vrgdaBatch.threePriceDecayPercent > 0 &&
      vrgdaBatch.threePerTimeUnit > 0,
      "VRGDA_THREE_MISSING_DATA"
    );
    require(
      vrgdaBatch.fourTargetPrice > 0 &&
      vrgdaBatch.fourPriceDecayPercent > 0 &&
      vrgdaBatch.fourPerTimeUnit > 0,
      "VRGDA_FOUR_MISSING_DATA"
    );
    require(
      vrgdaBatch.fiveTargetPrice > 0 &&
      vrgdaBatch.fivePriceDecayPercent > 0 &&
      vrgdaBatch.fivePerTimeUnit > 0,
  ...
  function totalNamesSold() public view returns (uint256) {
    return (
      soldCounts.one + soldCounts.two + soldCounts.three + 
        soldCounts.four + soldCounts.five + soldCounts.sixOrMore
    );
  }
